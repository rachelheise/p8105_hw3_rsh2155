...
are on Sundays, activity was quite high between 9 am and noon, and on
Fridays, activity was even higher between 8 pm and 10 pm. In general,
...
the rows of data only contain data on one of the precipitation and
...
snowfall commonly observed - count\! 2 panel plot: data manipulation
steps, then plotting step. group\_by(station, year, month) and
summarize(average max temp), filter for jan and july. Separate line for
each station. Use faceting for two panel plot.
